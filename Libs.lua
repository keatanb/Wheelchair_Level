...
function console:log(message)
	WheelchairLevel.Messages:Debug(message)
end
...
function WheelchairLevel.Lib:strcount(needle, heystack)
	local index = 1
	local count = 0
...
function WheelchairLevel.Lib:ConvertGlobalToRegexp(input)
...
function WheelchairLevel.Lib:GetFactionInfoByName(searchName)
...
function WheelchairLevel.Lib:GetRepLevelName(currentRep)
...
function WheelchairLevel.Lib:GetRepGainsToLevel(gain, lvlStart, lvlAmount, currentRep)
...
-- @param targetName The name of the target rep "level". If not valid, the function WheelchairLevel.Lib:returns nil.
function WheelchairLevel.Lib:GetRepGainsToTarget(gain, currentRep, targetName)
...
---
function WheelchairLevel.Lib:ZoneID()
...
function WheelchairLevel.Lib:ConvertClassification(classification)
	if type(classification) == "number" and classification > 0 and classification <= #WheelchairLevel.UNIT_CLASSIFICATIONS then
		return WheelchairLevel.UNIT_CLASSIFICATIONS[classification]
	elseif type(classification) == "string" then
		local out
		for i, v in ipairs(WheelchairLevel.UNIT_CLASSIFICATIONS) do
...
---
function WheelchairLevel.Lib:IsPlayerRafEligable()
...
---
function WheelchairLevel.Lib:IsRafApplied()
	return WheelchairLevel.db.profile.general.rafEnabled and self:IsPlayerRafEligable()
end

---
-- Used to find the Zero Difference used to calculate XP from low level mobs
---
function WheelchairLevel.Lib:ZeroDifference(charLevel)
	for i, v in ipairs(WheelchairLevel.ZD_Table) do
...
---
function WheelchairLevel.Lib:IsInBattleground()
	local currentZone = GetRealZoneText()
	for key, val in ipairs(WheelchairLevel.BG_NAMES) do
...
function WheelchairLevel.Lib:ShowBattlegroundData()
	return ((WheelchairLevel.db.profile.ldb.tooltip.showBGInfo and WheelchairLevel.Player.level >= 10) and
		(WheelchairLevel.Player.level < WheelchairLevel.Player.maxLevel or (#WheelchairLevel.db.char.data.bgList) > 0))
end

function WheelchairLevel.Lib:ShowDungeonData()
	return ((WheelchairLevel.db.profile.ldb.tooltip.showDungeonInfo) and
		(WheelchairLevel.Player.level < WheelchairLevel.Player.maxLevel or (#WheelchairLevel.db.char.data.dungeonList) > 0))
end

---
-- Calculates the XP gained from killing a mob
---
function WheelchairLevel.Lib:MobXP(charLevel, mobLevel, mobClassification)
	...
	if
		type(WheelchairLevel.db.char.data.npcXP[charLevel]) == "table" and
			type(WheelchairLevel.db.char.data.npcXP[charLevel][mobLevel]) == "table" and
			type(WheelchairLevel.db.char.data.npcXP[charLevel][mobLevel][mobClassIndex]) == "table" and
			#WheelchairLevel.db.char.data.npcXP[charLevel][mobLevel][mobClassIndex] > 0
	 then
		local high = 0
		for i, v in ipairs(WheelchairLevel.db.char.data.npcXP[charLevel][mobLevel][mobClassIndex]) do
			if v > high then
				high = v
			end
		end
	...
	elseif charLevel == mobLevel and mobClassIndex == 1 then
		...
		local zoneID = WheelchairLevel.Lib:ZoneID()
...
function WheelchairLevel.Lib:GetHeirloomXpBonus()
...
function WheelchairLevel.Lib:GetChatXPRegexp(isQuest)
...
---
function WheelchairLevel.Lib:ParseChatXPMessage(message, isQuest)
	local pattern = WheelchairLevel.Lib:GetChatXPRegexp(isQuest)
...
---
function WheelchairLevel.Lib:round(input, precision, roundDown)
	if input == nil then
		return nil
	end
...
---
function WheelchairLevel.Lib:NumberFormat(input) -- /run WheelchairLevel.Messages:Print(NumberFormat(127))
...
function WheelchairLevel.Lib:ShrinkNumber(input)
	input = tonumber(input)
	if input < 100000 then
		return WheelchairLevel.Lib:NumberFormat(input)
	else
		-- local units = {"", "K", "M", "B"}
		local index = 1
		local output = input
		...
		return WheelchairLevel.Lib:NumberFormat(tostring(WheelchairLevel.Lib:round(output, precision, true))) .. numberUnits[index]
	end
end
...
---
function WheelchairLevel.Lib:DecToHex(IN, minChars)
...
---
WheelchairLevel.Lib.progressColor = { pro = 0, hex = 0, rgb = { r = 0, g = 0, b = 0}}
function WheelchairLevel.Lib:GetProgressColor(pro)
	if pro <= 0 then
		pro = 1
	...
	if self.progressColor.pro ~= pro then
		...
		self.progressColor.hex =
			WheelchairLevel.Lib:DecToHex(self.progressColor.rgb.r, 2) ..
			WheelchairLevel.Lib:DecToHex(self.progressColor.rgb.g, 2) .. WheelchairLevel.Lib:DecToHex(self.progressColor.rgb.b, 2)
	end
	return self.progressColor.hex, self.progressColor.rgb
end
WheelchairLevel.Lib.progressColorS = { pro = 0, proa = 0, hex = 0, rgb = { r = 0, g = 0, b = 0}} -- TODO: Complete and test
function WheelchairLevel.Lib:GetProgressColor_Soft(progress)
	local hex
	...
	hex = WheelchairLevel.Lib:DecToHex(rgb.r, 2) .. WheelchairLevel.Lib:DecToHex(rgb.g, 2) .. WheelchairLevel.Lib:DecToHex(rgb.b, 2)
	return hex, rgb
end

function WheelchairLevel.Lib:GetDifficoultyColor(levelDifference)
end

---
-- Gets the minimum amount of XP that would be considered a valid BG objective reward.
---
function WheelchairLevel.Lib:GetBGObjectiveMinXP()
	if WheelchairLevel.Player.level > 10 then
		local bgMin = {
			["Alterac Valley"] = 750,
		...
		local playerMultiplier = (WheelchairLevel.Player.level - 10) / (WheelchairLevel.Player.maxLevel - 10)

		return (WheelchairLevel.Lib:round(zoneMin * playerMultiplier, 0))
	else
		return 0
	end
end
...
---
function WheelchairLevel.Lib:FindAnchor(frame)
...
function WheelchairLevel.Lib:ReverseAnchor(anchor)
...
---
function WheelchairLevel.Lib:Split(str, delim, maxNb)
	-- Eliminate bad cases...
...
function WheelchairLevel.Lib:TimeFormat(timestamp)
